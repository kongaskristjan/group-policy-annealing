...
def run_experiment(args: Namespace) -> list[float]:
    ...
    for step in range(args.anneal_steps):
        ...
        new_observations, new_actions, new_rewards, new_valid_mask = sample_batch_episode(model, envs)
        replay_buffer.add(new_observations, new_actions, new_rewards, new_valid_mask)
        observations, actions, rewards, valid_mask = replay_buffer.sample(args.training_batch_size)
        ...
        mean_reward = torch.mean(torch.sum(new_rewards * new_valid_mask, dim=1))
        new_ep_length = torch.mean(torch.sum(new_valid_mask, dim=1, dtype=torch.float32))
